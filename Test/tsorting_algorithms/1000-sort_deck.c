 ...
 *              If the characters are equal, it moves to the next pair of characters. 
 *              When it finds a pair of characters that are not equal, 
 *              it returns the difference between their ASCII values. 
 *              If it reaches the end of the strings without finding any unequal pair, 
 *              it returns 0, indicating that the strings are equal.
 *
 * Return: The difference between the ASCII values of the first pair of unequal characters.
 ...
 *
 * Return: The value of the card. If the card's name does not match any of the
 *         expected names, the function returns 13, which is the value for "King".
 */
char get_value(deck_node_t *card)
{
...
/**
 * insertion_sort_deck_kind - a function that sorts a deck of cards by kind using the insertion sort algorithm.
 * @deck: A double pointer to the head of the deck.
 *
 * Description: This function sorts a deck of cards by kind using the insertion sort algorithm. 
 *              It iterates through the deck, and for each card, it finds the correct position 
 *              for the card in the already sorted part of the deck and inserts it there.
 *
 * Return: This function does not return a value. It modifies the input deck in-place.
 */
void insertion_sort_deck_kind(deck_node_t **deck)
{
...
/**
 * insertion_sort_deck_value - Sorts a doubly linked list of cards in ascending order based on the 'value' attribute of the card.
 * If two cards have the same 'kind', their order is determined by their 'value'.
 *
 * @deck: A pointer to a pointer to the first node of the doubly linked list. 
 *              The list is sorted in-place, so the pointer to the first node may be modified.
 *
 ...
 * 
 * The function first checks if the deck is not empty and has more than one node.
 * If these conditions are met, it sorts the deck by kind and value using the 
 * insertion sort algorithm.
 *
 * @deck A double pointer to the head of the deck. The deck is a linked list
 *             where each node represents a card.
 * 
 * Return : void This function does not return a value. It modifies the deck in place.
 */
void sort_deck(deck_node_t **deck)
{
