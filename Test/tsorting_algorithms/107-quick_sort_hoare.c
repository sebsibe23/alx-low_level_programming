 ...
 *              a different logic to swap the values without a temporary variable.
 *
 ...
 *
 * Description: This function performs the Hoare partition scheme on an array. It uses
 *              a pivot element to partition the array into two halves, one with elements
 *              less than the pivot and one with elements greater than the pivot.
 *
 * Return: The final index of the pivot element after the partition.
 */
int hoare_partition(int *array, size_t size, int left, int right)
{
	int pivot;
...
/**
 * quick_sort_hoare - a function that performs quick sort on an array using the Hoare partition scheme.
 * @array: An array of integers to be sorted.
 * @size: The size of the array.
 *
 * Description: This function performs a quick sort on an array using the Hoare partition scheme. It uses
 *              a recursive approach to divide the array into two halves and sort them in opposite directions, 
 *              then merge them.
 *
 * Return: This function does not return a value. It modifies the input integers in-place.
 */
